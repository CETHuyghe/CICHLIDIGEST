...
do
    ...
    sbatch --job-name=index_${sample} --partition=scicore --account=salzburg index_sample.sh -o $index_dir -a $assembly
done
...
then
    ...
    tail -n +2 $simka_file | while read -r asmbl sam
        do
            ...
            # run the backmapping
            sbatch --job-name=backmap_assembly-${asmbl}_sample-${sam} --partition=scicore --account=salzburg backmap.sh -1 $R1 -2 $R2 -i $index -o $out_dir -s $sam  
    done
fi
...
for asmbl in $samples; do
    ...
    # bin the MAGs
    echo -e "\n\tBinning $asmbl"
    sbatch --job-name=bin_assembly-$asmbl --partition=scicore --account=salzburg bin_MAGs.sh -i $tmp/$asmbl -o $out_dir -a $asmbl -d $assembly 
done
