...
if [ ! -f $out_dir/simka.done ]
    then
        ...
        sbatch --job-name=simka --partition=scicore --account=salzburg --qos=1day --time=1-00:00:00 simka.sh -i $list_file -o $out_dir -m $maxreads -a $abund -c $maxcount -d $maxmerge
    else
