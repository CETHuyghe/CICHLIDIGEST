...
do
    ...
    sbatch --job-name=megahit_$sample --partition=scicore --account=salzburg megahit.sh -1 $R1 -2 $R2 -p $sample -o $out_dir
done
...
do
   ...
   # run quast
    sbatch --job-name=quast_$sample --partition=scicore --account=salzburg quast.sh -i $out/assemblies/$sample/${sample}.contigs.fa -o $out_dir
done
